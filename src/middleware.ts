...
const PUBLIC_API = ["/api/auth/login", "/api/auth/logout", "/api/health", "/api/db-check", "/api/migrate", "/api/brands", "", "/api/categorias", "/api/solved/public-create", "/api/debug-env"];
...
export const config = {
  matcher: ["/app/:path*", "/api/:path*", "/admin/:path*", "/envios/:path*", "/nuevo-paquete/:path*", "/hbl/:path*", "/portal/:path*", "/factura/:path*", "/almacen/:path*", "/combustible/:path*"],
};
