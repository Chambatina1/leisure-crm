        ...
        {filtered.length === 0 ? (
          ...
          return (
            <Card key={product.id} className="border-0 shadow-md hover:shadow-lg transition-shadow overflow-hidden">
              {hasImage ? (
                <div className="w-full h-48 relative bg-white flex items-center justify-center overflow-hidden">
                  <img src={product.imagenUrl!} alt={product.nombre} loading="lazy" decoding="async" className="w-full h-full object-contain" onError={() => setImgErrors(prev => new Set(prev).add(product.id))} />
                </div>
              ) : (
